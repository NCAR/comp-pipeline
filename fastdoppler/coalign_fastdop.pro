...
;  Modified by: Tomczyk
;-
pro coalign_fastdop, date_dir, list_file = list_file
...
;  create output fits file (with .ca extension)
  ...
  out_name=process_dir+'/'+filenames[ifile]+'.ca.fts'
...
;  loop over all extensions and align each image with same aia image
  ...
  restore,file=string(process_dir+'/'+filenames[ifile]+'.ca.sav')   ;restore offsets
