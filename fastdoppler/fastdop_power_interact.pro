;+
;  Name: fastdop_power_interact
;
;  Description:
;    Procedure to plot power spectra of CoMP fast doppler data interactively.
;
;  Input Parameters:
...
;
;  Examples:
;    fastdop_power_interact, '20140102', '120956'
;-
pro fastdop_power_interact, date_dir, time_str
...
good=where(msk eq 1. and avg_intens gt 4.,complement=bad,count)
print,count
msk[bad]=0.
...
while !mouse.button ne 4 do begin
  ...
  print,'move cursor to region to trace and press left button (right to stop)'
  ...
  dx=2        ;box radius
  for i=-dx,dx do for j=-dx,dx do begin
...
;    avg_i_pow=avg_i_pow+pow
  ...
  wset,2
  plot,time[0:50],v[0:50],xtit='Time (s)',ytit='Velocity (km/s)',yr=[-5,5],tit=string(stdev(v))

  wset,3
  plot_oo,freq,avg_v_pow,xtit='Frequency (Hz)',ytit='Power',xr=[1.e-3,1.],yr=[1.e-4,0.1]
