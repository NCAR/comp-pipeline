...
openw,1,'C:\Users\tomczyk\Documents\Comp\idl\Systematics\out.txt'
...
;  look for flat file for this day
  ...
  if r ne '' && info.size gt 3072 then begin   ;if flat exists, then process it    
    calibrate_comp_wavelength_2,date_dir,lam0,offset,h2o,flat_time,off_tell,dev=dev
  
    if flat_time[0] ne 0. then begin
      nflat=n_elements(flat_time)
      for i=0,nflat-1 do begin
        printf,1,date_dir,flat_time[i],offset[i,0],h2o[i,0],off_tell[i,0]
        printf,1,date_dir,flat_time[i],offset[i,1],h2o[i,1],off_tell[i,1]
      endfor
    endif
  endif
