...
;   MLSO Software Team
;-
pro comp_average_insert, date, wave_type, type, $
                         database=db, obsday_index=obsday_index
  compile_opt strictarr
  @comp_config_common

  basename = string(date, wave_type, type, format='(%"%s.comp.%s.%s.*.fts*")')
  average_files = file_search(filepath(basename, $
                                            subdir=[date, 'level2'], $
  ...
  if (n_average_files gt 0L) then begin
    mg_log, 'inserting %d rows into %s nm comp_file table...', $
            n_average_files, wave_type, $
            name='comp', /info
  endif else begin
    mg_log, 'no %s average files to insert into %s nm comp_file table', $
            type, wave_type, $
            name='comp', /info
    goto, done
  endelse

  ; get average product type from mlso_producttype
  producttype = comp_get_producttype_id(type, database=db)
  ...
  for f = 0L, n_average_files - 1L do begin
    ...
    db->execute, sql_cmd, $
                 file_basename(average_files[f], '.gz'), $
                 date_obs, $
                 obsday_index, $
...
comp_average_insert, date, '1074', 'mean', $
                     database=db, obsday_index=obsday_index
comp_average_insert, date, '1074', 'median', $
                     database=db, obsday_index=obsday_index
comp_average_insert, date, '1079', 'mean', $
                     database=db, obsday_index=obsday_index
comp_average_insert, date, '1079', 'median', $
                     database=db, obsday_index=obsday_index
