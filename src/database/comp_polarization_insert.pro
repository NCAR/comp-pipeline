...
;   MLSO Software Team
;-
pro comp_polarization_insert, date, wave_type, $
                              database=db, obsday_index=obsday_index
  compile_opt strictarr
  @comp_config_common

  basename = string(wave_type, format='(%"*.comp.%s.polarization.fts*")')
  polarization_files = file_search(filepath(basename, $
                                            subdir=[date, 'level2'], $
  ...
  if (n_polarization_files gt 0L) then begin
    mg_log, 'inserting %d rows into %s nm comp_file table...', $
            n_polarization_files, wave_type, $
            name='comp', /info
  endif else begin
    mg_log, 'no polarization files to insert into %s nm comp_file table', $
            wave_type, $
            name='comp', /info
    goto, done
  endelse

  ; get polarization product type from mlso_producttype
  producttype = comp_get_producttype_id('polarization', database=db)
  ...
  for f = 0L, n_polarization_files - 1L do begin
    ...
    db->execute, sql_cmd, $
                 file_basename(polarization_files[f], '.gz'), $
                 date_obs, $
                 obsday_index, $
...
comp_polarization_insert, date, '1074', $
                          database=db, obsday_index=obsday_index
comp_polarization_insert, date, '1079', $
                          database=db, obsday_index=obsday_index
