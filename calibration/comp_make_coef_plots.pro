...
;+
; Makes plots of the crosstalk coefficients product by the calibration code,
; using information stored in the common block. Produces one plot file for
; each polarization analyzer state, containing plots of the to each component
; of the stokes vector (I, Q, U, and V).
;
; :Params:
;   plot_directory : in, required, type=string
;     the name of the directory to which the plots should be written
;
; :Author:
;   Joseph Plowman
;-
pro comp_make_coef_plots, plot_directory
  ...
  ; loop over unique polarizations, i.e., analyzer states; nominally I+Q, I+U,
  ; etc)
  for i = 0, nupols - 1 do begin
    ...
    for j = 0,nstokes - 1 do begin   ; loop over Stokes vector components
      ...
      plot_image, coef_image, $
                 title=stokeslabels[j] + ', min=' + min_str + ', max=' + max_str, $
                 top=254,$
                 bottom=1,$
                 min=-pmax,$
                 max=pmax
