...
;     ------------ define file name and contents -----------------------
      temppro=get_logenv('MS_TEMP')
      home=get_logenv('HOME')  & delim=path_sep()		; all os parameters
      if temppro eq '' or not test_dir(temppro,/quiet) then begin
      ...
      endif else temppro = concat_dir(temppro, str_name + '.pro')
...
;     ------ set up the path and execute the function ---------------------
      ...
      cd, file_dirname(temppro), current=current_dir
      qsave = !quiet
      !quiet = 1
